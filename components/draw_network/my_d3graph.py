    ...
    * Documentation: https://erdogant.github.io/d3graph/
    ...
    def set_node_properties(
        ...
        if len(tooltip) != nodecount:
            raise ValueError(
                "[tooltip text] must be of same length as the number of nodes"
            )
        ...
        if len(edge_color) != nodecount:
            raise ValueError(
                "[edge_color] must be of same length as the number of nodes"
            )
        ...
        edge_size = _normalize_size(
            edge_size.reshape(-1, 1), 0.5, 4, scaler=self.config["node_scaler"]
        )
        if len(edge_size) != nodecount:
            raise ValueError(
                "[edge_size] must be of same length as the number of nodes"
            )
    ...
    def get_cluster_color(
        self, node_names: list = None, color: str = "#000080"
    ) -> tuple:
        ...
        # Set defaults
        labx = {
            key: {"name": key, "color": color, "group": "-1"}
            for i, key in enumerate(node_names)
        }
    ...
    def setup_slider(self) -> None:
        ...
        logger.info(
            "Slider range is set to [%g, %g]"
            % (self.config["slider"][0], self.config["slider"][1])
        )
    ...
    def graph(
        ...
        self.set_node_properties(
            color=color, opacity=opacity, size=size, scaler=scaler, cmap=cmap
        )
    ...
    def write_html(self, json_data, overwrite: bool = True) -> None:
        ...
        click_properties = {
            **{
                "fill": "function(d) {return d.node_color;}",
                "stroke": "black",
                "size": 1.3,
                "stroke-width": 3,
            },
        ...
        width = (
            "window.screen.width"
            if self.config["figsize"][0] is None
            else self.config["figsize"][0]
        )
        height = (
            "window.screen.height"
            if self.config["figsize"][1] is None
            else self.config["figsize"][1]
        )
        ...
        if overwrite and index_file:
            logger.info(f"Write to path: [{index_file}]")
    ...
    def import_example(self, data="energy", url=None, sep=","):
        ...
        if data == "small":
            ...
            adjmat = pd.DataFrame(
                index=range(adjmat.shape[0]),
                data=adjmat,
                columns=range(adjmat.shape[0]),
            )
...
def adjmat2dict(
    ...
    if minmax_distance is not None:
        df["edge_distance"] = _normalize_size(
    ...
    # Scale the weights
    if (
        len(np.unique(df["weight"].values.reshape(-1, 1))) > 2 or (minmax is not None)
    ) and (scaler is not None):
        df["weight_scaled"] = _normalize_size(
            df["weight"].values.reshape(-1, 1), minmax[0], minmax[1], scaler=scaler
        )
    ...
    if label is None:
        ...
        label = list(
            map(lambda x: ("%f" % x).rstrip("0").rstrip("."), df["weight"].values)
        )
    ...
    if len(source_target) == 0:
        raise Exception(
            "There are no links in the input data set that have unique source-target value with weight > 0"
        )
    # Return
    d = {
        edge: {
    ...
    if return_adjmat:
        return d, vec2adjmat(
            df["source"], df["target"], weight=df["weight"], symmetric=True
        )
    else:
        return d
        ...
        for col in node_properties.columns:
            for i in range(node_properties.shape[0]):
                ...
                if np.any(np.isin(getnodes, node_properties.index.values[i])):
                    ...
                    logger.warning(
                        f"Node [{node_properties.index.values[i]}] not found"
                    )
    return G
...
def data_checks(adjmat: pd.DataFrame) -> pd.DataFrame:
    ...
    if not np.all(adjmat.columns == adjmat.index.values):
        raise ValueError(
            logger.error("adjmat columns and index must have the same identifiers")
        )
...
# %%  Convert adjacency matrix to vector
def vec2adjmat(
    source, target, weight=None, symmetric: bool = True, aggfunc="sum"
) -> pd.DataFrame:
    ...
    adjmat = pd.crosstab(
        df["source"], df["target"], values=weight, aggfunc=aggfunc
    ).fillna(0)
...
def _check_hex_color(color, n=None):
    ...
    if isinstance(color, list) and (
        not np.all(list(map(lambda x: len(x) == 7, color)))
    ):
        raise ValueError(
    ...
    if (n is not None) and isinstance(color, list) and len(color) != n:
        raise ValueError(
            f"Input parameter [color] has wrong length. Must be of length: {str(n)}"
        )
...
def _compute_centrality(adjmat):
    ...
    # Normalize
    # opacity = opacity + [0, 1]
    opacity = _normalize_size(
        np.array(opacity).reshape(-1, 1), 0.35, 0.99, scaler="zscore"
    )
    # opacity = opacity[:-2]
    return opacity
...
def _set_node_size(self, size, minmax, nodecount):
    ...
    size = _normalize_size(
        size.reshape(-1, 1), minmax[0], minmax[1], scaler=node_scaler
    )
