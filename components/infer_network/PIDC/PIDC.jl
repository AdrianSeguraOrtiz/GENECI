...
elseif length(ARGS) == 1 && ARGS[1] == "--help" 
    println("Usage:")
    println("julia PIDC.jl input.csv path/to/output_folder") 
...
inferred_network = infer_network(in_file, PIDCNetworkInference(); delim = ',')

max_weight = first(inferred_network.edges).weight
min_weight = last(inferred_network.edges).weight

out_file = open(output_folder * "/GRN_PIDC.csv", "w")
