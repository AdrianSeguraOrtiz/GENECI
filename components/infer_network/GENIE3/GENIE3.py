...
def genie3(
    ...
    regressor_type: Regressor = typer.Argument(
        ...,
        help="Regression type: Random Forest regression (RF), Gradient Boosting Machine regression with early-stopping regularization (GBM) or ExtraTrees regression (ET)",
    ),
):
    ...
    filename = (
        ...
        + (
            "GRNBOOST2"
            if regressor_type.name == "GBM"
            else ("GENIE3_" + regressor_type.name)
        )
        + ".csv"
    )
