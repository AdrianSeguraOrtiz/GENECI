...
if (length(ARGS) >= 2) { 
    ...
    cat("Usage: \n")
    cat("Rscript GENIE3.R input.csv path/to/output_folder \n") 
...
write.table(conf_list_RF, paste0("./", output_folder, "/", file_id, "/lists/GRN_GENIE3_RF.csv"), sep=",", col.names=F, row.names=F, quote=F)
write.table(conf_list_ET, paste0("./", output_folder, "/", file_id, "/lists/GRN_GENIE3_ET.csv"), sep=",", col.names=F, row.names=F, quote=F)
