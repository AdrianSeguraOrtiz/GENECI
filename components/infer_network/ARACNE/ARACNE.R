...
if (length(ARGS) >= 2) { 
    ...
    cat("Usage: \n")
    cat("Rscript ARACNE.R input.csv path/to/output_folder \n") 
...
write.table(conf_list, paste0("./", output_folder, "/", file_id, "/lists/GRN_ARACNE.csv"), sep=",", col.names=F, row.names=F, quote=F)
