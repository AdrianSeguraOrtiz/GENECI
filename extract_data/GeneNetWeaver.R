...
for (str_n in v.str_networks) {
    ...
    write.table(mtx.exp, paste0("../expression_data/GeneNetWeaver/EXP/", str_n, "_exp.csv"), sep=",", col.names = NA)

    # Extract gold standard adjacency matrix
    mtx.gs <- l.data[[2]]

    # Save gold standard
    write.table(mtx.gs, paste0("../expression_data/GeneNetWeaver/GS/", str_n, "_gs.csv"), sep=",", col.names = NA)
}