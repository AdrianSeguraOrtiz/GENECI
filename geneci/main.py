...
def cluster_network(
    ...
    container = client.containers.run(
        ...
        command=f"--confidence-list {tmp_confidence_list_dir} --algorithm {algorithm.value} --preferred-size {preferred_size} --min-size {min_size} --output-folder {temp_folder_str}",
        detach=True,
        tty=True,
    )
