...
def join_scores(
    ...
    best_row = pd.DataFrame(
        [["BEST_GENECI"] + best_row.tolist()],
        columns=["Technique"] + best_row.index.tolist(),
    )
    ...
    median_row = pd.DataFrame(
        [["MEDIAN_GENECI"] + median_row.tolist()],
        columns=["Technique"] + median_row.index.tolist(),
    )
    ...
    if mean_file:
        ...
        df_tecs = pd.concat(
            [
                df_tecs,
                pd.DataFrame([["MEAN"] + mean], columns=df_tecs.columns.tolist()),
            ],
            ignore_index=True,
        )
    ...
    if median_file:
        ...
        df_tecs = pd.concat(
            [
                df_tecs,
                pd.DataFrame([["MEDIAN"] + median], columns=df_tecs.columns.tolist()),
            ],
            ignore_index=True,
        )
    ...
    if rank_file:
        ...
        df_tecs = pd.concat(
            [
                df_tecs,
                pd.DataFrame(
                    [["RANK_AVERAGE"] + rank], columns=df_tecs.columns.tolist()
                ),
            ],
            ignore_index=True,
        )
    ...
    if bayesian_file:
        ...
        df_tecs = pd.concat(
            [
                df_tecs,
                pd.DataFrame(
                    [["BAYESIAN_FUSION"] + bayesian], columns=df_tecs.columns.tolist()
                ),
            ],
            ignore_index=True,
        )
