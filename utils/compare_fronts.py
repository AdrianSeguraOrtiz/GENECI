...
def add_3d_poblation(
    ...
    fig.add_trace(
        ...
        row=trace_position[0],
        col=trace_position[1],
    )


def add_3d_ref_point(
    fig, ref_point: list, color: str, trace_label: str, trace_position: list
):
    fig.add_trace(
        go.Scatter3d(
            x=[ref_point[1]],
...
def add_half_violin(
    fig, ef: pd.DataFrame, column: str, color: str, side: str, trace_position: list
):
    fig.add_trace(
        go.Violin(
            x=[0] * len(ef),
...
def main(
    ...
    for ref_point_csv in ref_points_csv:
        ref_points.append(
            list(pd.read_csv(ref_point_csv, sep=";", header=None).loc[0, :])
        )

    ref_points_rows = []
    for ref_point in ref_points:
        ref_points_rows.append(
            orig_ef.loc[
    ...
    for i in range(len(ref_efs)):
        col = next(
            (
                ...
                if metric.lower() in ref_points[i][0].lower()
            ),
            "Accuracy Mean",
        )
        add_3d_poblation(
            ...
            col,
            palettes[i],
            [bar_len / 2 + (i + 1) * bar_len, -0.1, bar_len],
            [1, 1],
            f"Reference Point Front {i+1} ({ref_points[i][0]})",
        )
        add_3d_ref_point(
            fig_combined,
            ref_points[i],
            colors[i],
            f"Reference Point {i+1} ({ref_points[i][0]})",
            [1, 1],
        )
    ...
    for i in range(len(ref_efs)):
        if "aupr" in ref_points[i][0]:
            add_half_violin(
                fig_combined, ref_efs[i], "AUPR", colors[i], "positive", [1, 2]
            )
            if not ref_points_rows[i].empty:
                add_violin_point(
                    fig_combined, ref_points_rows[i].iloc[0]["AUPR"], colors[i], [1, 2]
                )
    ...
    for i in range(len(ref_efs)):
        if "auroc" in ref_points[i][0]:
            add_half_violin(
                fig_combined, ref_efs[i], "AUROC", colors[i], "positive", [1, 3]
            )
            if not ref_points_rows[i].empty:
                add_violin_point(
                    fig_combined, ref_points_rows[i].iloc[0]["AUROC"], colors[i], [1, 3]
                )
    ...
    columns = ["Original"] + [
        f"Ref Point {ref_points[i][0]}" for i in range(len(ref_points))
    ]
    df_aupr = pd.DataFrame(columns=columns)
    df_aupr.loc[0] = [orig_ef["AUPR"].median()] + [
        ref_efs[i]["AUPR"].median() for i in range(len(ref_efs))
    ]
    df_auroc = pd.DataFrame(columns=columns)
    df_auroc.loc[0] = [orig_ef["AUROC"].median()] + [
        ref_efs[i]["AUROC"].median() for i in range(len(ref_efs))
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--initial-evaluated-front",
        type=str,
        required=True,
        help="Path to the initial evaluated front CSV file.",
    )
    parser.add_argument(
        "--ref-point-evaluated-fronts",
        nargs="+",
        required=True,
        help="Path to the reference point evaluated front CSV file.",
    )
    parser.add_argument(
        "--ref-points-csv",
        nargs="+",
        required=True,
        help="Path to the reference point CSV file.",
    )
    parser.add_argument(
