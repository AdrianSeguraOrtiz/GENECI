 ...
 * @param <S>
 */
public class GNSGAIIBuilder<S extends Solution<?>> {
  private String name;
  ...
  public GNSGAIIBuilder(Problem<S> problem, int populationSize, int offspringPopulationSize,
    ...
    this.replacement =
        new OldRankingAndDensityEstimatorReplacement<>(
            ranking, densityEstimator, Replacement.RemovalPolicy.ONE_SHOT);

    this.variation =
        new OldCrossoverAndMutationVariation<>(
  ...
  public GNSGAIIBuilder<S> setTermination(Termination termination) {
    this.termination = termination;

    return this;
  }

  public GNSGAIIBuilder<S> setRanking(Ranking<S> ranking) {
  ...
  public GNSGAIIBuilder<S> setEvaluation(Evaluation<S> evaluation) {
