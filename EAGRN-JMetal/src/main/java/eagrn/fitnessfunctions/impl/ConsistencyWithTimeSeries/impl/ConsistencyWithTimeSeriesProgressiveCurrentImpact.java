...
public class ConsistencyWithTimeSeriesProgressiveCurrentImpact extends ConsistencyWithTimeSeries {
    
    public ConsistencyWithTimeSeriesProgressiveCurrentImpact(Map<String, Double[]> timeSeriesMap) {
        super(timeSeriesMap);
        ...
        for (Map.Entry<String, Double[]> tsPair : super.timeSeriesMap.entrySet()) {
            ...
            for (int i = 0; i < tsPair.getValue().length - 1; i++) {
                ...
                for (Map.Entry<String, Double> factor : factors.entrySet()) {
                    prediction += super.variationsMap.get(factor.getKey())[i] * factor.getValue();
