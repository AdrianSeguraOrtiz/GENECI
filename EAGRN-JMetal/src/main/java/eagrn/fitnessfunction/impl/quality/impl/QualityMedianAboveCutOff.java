...
public class QualityMedianAboveCutOff extends Quality {
    private CutOffCriteria cutOffCriteria;
    
    public QualityMedianAboveCutOff(Map<String, Float[]> inferredNetworks, CutOffCriteria cutOffCriteria) {
    ...
    public double run(Map<String, Float> consensus, Double[] x) {
        ...
        Map<String, Float> goodLinks = cutOffCriteria.getCutMap(consensus);

        /** 3. Calculate the average of confidence-distance means of high quality links */
        float conf, dist, confDistSum = 0;
        for (Map.Entry<String, Float> pair : goodLinks.entrySet()) {
            conf = pair.getValue();
