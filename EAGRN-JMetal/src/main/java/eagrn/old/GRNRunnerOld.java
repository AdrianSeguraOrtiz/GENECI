...
public class GRNRunnerOld extends AbstractAlgorithmRunner {
    /**
    ...
    public static void main(String[] args) throws IOException {
        ...
        repairer = StaticUtilsOld.getRepairerFromString(strRepairer);

        /** List CSV files stored in the input folder with inferred lists of links. */
        File[] files = StaticUtilsOld.getCSVFilesFromDirectory(networkFolder + "/lists/");

        /** Extract inferred networks */
        Map<String, Double[]> inferredNetworks = StaticUtilsOld.readAllInferredNetworkFiles(files);

        /** Extracting gene names. */
        ArrayList<String> geneNames = StaticUtilsOld.getGeneNames(networkFolder + "/gene_names.txt");
        ...
        cutOffCriteria = StaticUtilsOld.getCutOffCriteriaFromString(strCutOffCriteria, cutOffValue, geneNames);
        ...
        crossover = StaticUtilsOld.getCrossoverOperatorFromString(strCrossover, crossoverProbability, crossoverDistributionIndex, numPointsCrossover);
        ...
        mutation = StaticUtilsOld.getMutationOperatorFromString(strMutation, mutationProbability, repairer, mutationDistributionIndex, delta, numberOfGroups, grouping, perturbation, maxMutIterations);
        ...
        if (printEvolution) {
            ...
            StaticUtilsOld.writeFitnessEvolution(outputFolder + "/fitness_evolution.txt", fitnessEvolution);
        }
        ...
        if (problem.getNumberOfObjectives() == 1) {
            ...
            Map<String, Double> consensus = StaticUtilsOld.makeConsensus(winner, inferredNetworks);
            ...
            StaticUtilsOld.writeConsensus(outputFolder + "/final_list.csv", consensusSorted);
            ...
            StaticUtilsOld.writeBinaryNetwork(outputFolder + "/final_network.csv", binaryNetwork, geneNames);
        }
